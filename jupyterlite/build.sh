#!/usr/bin/env bash
#
# Build the pipescript JupyterLite demo site (the target of the "launch in
...
#   1. the pipescript wheel (runs in the browser via piplite),
...
# needed to make `%pip install pipescript` resolve them offline via piplite.
WHEELS="$HERE/pypi"
...
echo "==> Building the pipescript wheel from this checkout"
...
python -m pip download --only-binary=:all: \
  ...
  pyccolo comm traitlets typing-extensions -d "$WHEELS"
